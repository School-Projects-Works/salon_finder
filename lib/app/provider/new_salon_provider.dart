...
final newSalonProvider = StateNotifierProvider<NewSalonNotifier, NewSalonState>(
  (ref) => NewSalonNotifier(),
);

class NewSalonNotifier extends StateNotifier<NewSalonState> {
  NewSalonNotifier() : super(NewSalonState(salon: SalonModel.initial()));
  ...
  Future<void> saveSalon({
    ...
    state = state.copyWith(
      salon: state.salon.copyWith(
        id: SalonServices.generateSalonId(),
        createdAt: DateTime.now().millisecondsSinceEpoch,
        salonOwnerId: user.id,
      ),
    );
    var response = await SalonServices.createSalon(state.salon);
    if (response.success) {
      CustomDialog.closeDialog();
